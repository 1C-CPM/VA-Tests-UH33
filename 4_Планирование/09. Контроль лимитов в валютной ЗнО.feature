...
Сценарий: 09.02 Создать валютную Заявку на оплату (валют взаиморасчетов и платежа USD) 			
	...
	* Заполнить платежные реквизиты
		...
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			...
			И в таблице "Список" я выбираю текущую строку
		И я удаляю переменную 'ДатаОплаты'
	...
	* Заполнить аналитики учета и планирования
		Если '$$ЭтоУХ$$' Тогда
			...
			И в таблице 'ТаблицаДвижений' я нажимаю кнопку выбора у реквизита с именем 'ТаблицаДвиженийСтатьяДвиженияДенежныхСредств'
			...
			И в таблице "ТаблицаДвижений" в поле с именем "ТаблицаДвиженийПроект" я ввожу значение выражения 'ВАПроект'
			И я перехожу к следующему реквизиту
	...
	* Проверить движения
		...
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			...
			И Таблица 'Список' содержит '1' строк из списка:
				...
				| '$ДатаОплаты$ 0:00:00' | 'Заявка на оплату *; $ВАКонтрагент$; Оплата поставщику' | 'Бюджет движения денежных средств' | 'Заявка на оплату *; $ВАКонтрагент$; Оплата поставщику' | 'Расход'        | '$ВАЦФО$; $ВАПроект$; $ВАОрганизация$' | '$ВАСтатья$ ($ВААналитика1$;  - ;  - ;  - ;  - ;  - ) Расход' | '$ВАОрганизация$; $ВАКонтрагент$; 09.02*; 09.02*' | 'USD'    | 'RUB'                  | '$СуммаЛимитаВалюта$' | '$СуммаЛимита$' | ''                | ''                | ''                | '$$КурсОплатыПлан$$' | '1'                      | '1,0000'             | '1'                       |
...
Сценарий: 09.04 Создать валютную Заявку на оплату (валют взаиморасчетов USD и платежа RUB) 			
	...
	* Проверить движения
		...
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			И Я открываю движения документа с отбором по виду движений 'Регистр накопления Операции бюджетов' в ЕРП УХ
			И Таблица 'Список' содержит '1' строк из списка:
				| 'Период'               | 'Регистратор'                                           | 'Номер строки' | 'Вид бюджета'                      | 'Объект планирования'                                   | 'Приход расход' | 'Аналитика структуры'                  | 'Аналитика статей бюджетов'                                   | 'Аналитика партнеров'                             | 'Валюта' | 'Валюта лимитирования' | 'Сумма'               | 'Лимит'         | 'Сумма исполнено' | 'Лимит исполнено' | 'Источник версии' | 'Курс лимитирования' | 'Кратность лимитирования' |
