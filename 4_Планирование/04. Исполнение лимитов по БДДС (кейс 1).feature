	...
	* Проверить наследование аналитик в ПП
		Если '$$ЭтоУХ$$' Тогда 
			...
			Тогда таблица "РасшифровкаПлатежа" стала равной по шаблону:
				| 'N' | 'Договор'      | 'Сумма платежа' | 'Ставка НДС' | 'Сумма НДС' | 'ЦФО'              | 'Проект' | 'Статья ДДС'    | 'Аналитика 1' | 'Аналитика 2'       | 'Аналитика 3'       | 'Аналитика 4'       | 'Аналитика 5'       | 'Аналитика 6'       | 'Элемент структуры задолженности' | 'Документ планирования'                                                 |
				| '1' | 'Без договора' | '3 000,00'      | '22%'        | '540,98'    | 'ВА - ЦФО 1 (АХО)' | ''       | 'ВА - Статья 1' | 'ВА - Здания' | '<Не используется>' | '<Не используется>' | '<Не используется>' | '<Не используется>' | '<Не используется>' | 'Основной долг'                   | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' |
		...
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			...
			Тогда таблица "РасшифровкаПлатежа" стала равной по шаблону:
				...
				| '1' | '3 000,00'             | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' | '<Не используется>' | 'Основной долг'                   | '04.01.2 от '     | '<Не используется>' | '04.01.2 от ' | 'ВА - ЦФО 1 (АХО)' | 'RUB'    | '<Не используется>' | 'ВА - Поставщик1' | ''            | '3 000,00'    | 'ВА - Здания' | ''               | '22%'        | '540,98'          | 'ВА - Статья 1' | ''       | '<Не используется>' | '<Не используется>' |
...
Сценарий: 04.01.4 Отражаем фактические данные об оплате	 		 		  		 		 		 	 
	...
	* Провести банком
		Если '$$ЭтоУХ$$' Тогда 
			...
			Тогда таблица "РасшифровкаПлатежа" стала равной по шаблону:
				...
				| '1' | 'Без договора'              | 'ВА - Статья 1'   | ''       | ''            | ''                  | ''      | 'ВА - Здания' | 'Автоматически'           | '1'    | '3 000'          | 'Счет от поставщика * от *' | '60.02'        | '22%'        | '540,98' | '60.01'          | ''            | 'ВА - ЦФО 1 (АХО)' | ''            | ''            | ''            | 'Основной долг'                   | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' | ''               | ''       |
	...
	* Проверить движения ОФД	
		...
		Если '$$ЭтоУХ$$' Тогда 
			Тогда таблица "ОперацииБюджетов" стала равной по шаблону:
				| 'N' | 'Вид бюджета'                      | 'Приход расход' | 'Кратность лимитирования' | 'Объект планирования'                                                   | 'Сумма'     | 'Аналитика партнеров'                                        | 'Источник версии' | 'Лимит'     | 'Состояние' | 'Курс планирования' | 'Сумма исполнено' | 'Аналитика структуры'             | 'Лимит исполнено' | 'Валюта' | 'Валюта лимитирования' | 'Кратность планирования' | 'Курс задан в операции' | 'Курс лимитирования' | 'Аналитика статей бюджетов'                                   | 'Корректировка документа' |
				| '1' | 'Бюджет движения денежных средств' | 'Расход'        | '1'                       | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' | '-3 000,00' | 'ВА - ООО Тест; ВА - Поставщик1; Без договора; Без договора' | ''                | '-3 000,00' | ''          | '1,0000'            | '3 000,00'        | 'ВА - ЦФО 1 (АХО); ВА - ООО Тест' | '3 000,00'        | 'RUB'    | 'RUB'                  | '1'                      | 'Нет'                   | '1,0000'             | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | ''                        |
...
Сценарий: 04.02.3 Создаем Корректировку лимитов по заявке
	...
	* Обеспечить за счет переноса из другого периода
		...
		И в таблице "Список" я перехожу к строке:
			| 'Аналитика 1' | 'Дата операции' | 'Направление' | 'Статья бюджета' | 'ЦФО/Проект'       |
			| 'ВА - Здания' | 'Март 2024 г.'  | 'Расход'      | 'ВА - Статья 1'  | 'ВА - ЦФО 1 (АХО)' |
		И в таблице "Список" я выбираю текущую строку
