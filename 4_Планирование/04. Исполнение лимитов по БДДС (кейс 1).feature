	...
	* Проверить наследование аналитик в ПП
		Если '$$ЭтоУХ$$' Тогда 
			...
			Тогда таблица "РасшифровкаПлатежа" стала равной по шаблону:
				| 'N' | 'Договор'      | 'Сумма платежа' | 'Курс' | 'Расчеты в у.е.' | 'Сумма' | 'Ставка НДС' | 'Сумма НДС' | 'ЦФО'              | 'Проект' | 'Статья ДДС'    | 'Аналитика 1' | 'Аналитика 2'       | 'Аналитика 3'       | 'Аналитика 4'       | 'Аналитика 5'       | 'Аналитика 6'       | 'Элемент структуры задолженности' | 'Документ планирования'                                                 | 'Отступ' |
				| '1' | 'Без договора' | '3 000,00'      | '1'    | '3 000'          | ''      | '22%'        | '540,98'    | 'ВА - ЦФО 1 (АХО)' | ''       | 'ВА - Статья 1' | 'ВА - Здания' | '<Не используется>' | '<Не используется>' | '<Не используется>' | '<Не используется>' | '<Не используется>' | 'Основной долг'                   | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' | ''       |
