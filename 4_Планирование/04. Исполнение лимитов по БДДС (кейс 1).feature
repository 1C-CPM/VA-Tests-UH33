	...
	* Проверить наследование аналитик в ПП
		Если '$$ЭтоУХ$$' Тогда 
			...
			Тогда таблица "РасшифровкаПлатежа" стала равной по шаблону:
				...
				| '1' | 'Без договора' | '<Не используется>' | 'ВА - ЦФО 1 (АХО)' | '3 000,00'      | '1'    | '3 000'          | ''      | 'ВА - Здания' | '20%'        | '500,00'       | '<Не используется>' | ''       | 'ВА - Статья 1' | '<Не используется>' | '<Не используется>' | 'Основной долг'                   | '<Не используется>' | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' | ''       |
