	...
	* Проверить наследование аналитик в ПП
		Если '$$ЭтоУХ$$' Тогда 
			...
			Тогда таблица "РасшифровкаПлатежа" стала равной по шаблону:
				| 'N' | 'Договор'      | 'Аналитика 2'       | 'ЦФО'              | 'Сумма платежа' | 'Вал.' | 'Аналитика 5'       | 'Проект' | 'Договор контрагента валюта взаиморасчетов' | 'Курс' | 'Расчеты в у.е.' | 'Ставка НДС' | 'Сумма НДС' | 'Статья ДДС'    | 'Аналитика 1' | 'Аналитика 3'       | 'Аналитика 6'       | 'Аналитика 4'       | 'Элемент структуры задолженности' | 'Документ планирования'                                                              |
				| '1' | 'Без договора' | '<Не используется>' | 'ВА - ЦФО 1 (АХО)' | '3 000,00'      | ''     | '<Не используется>' | ''       | 'RUB'                                       | ''     | '3 000'          | '20%'        | '500,00'    | 'ВА - Статья 1' | 'ВА - Здания' | '<Не используется>' | '<Не используется>' | '<Не используется>' | 'Основной долг'                   | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' |
		...
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			...
			Тогда таблица "РасшифровкаПлатежа" стала равной по шаблону:
				...
				| '1' | '3 000,00'             | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' | '<Не используется>' | 'Основной долг'                   | '04.01.2 от '     | '<Не используется>' | '04.01.2 от ' | 'ВА - ЦФО 1 (АХО)' | 'RUB'    | '<Не используется>' | 'ВА - Поставщик1' | ''            | '3 000,00'    | 'ВА - Здания' | ''               | '20%'        | '500,00'          | 'ВА - Статья 1' | ''       | '<Не используется>' | '<Не используется>' |
	...
	* Провести банком
		Если '$$ЭтоУХ$$' Тогда 
			...
			Тогда таблица "РасшифровкаПлатежа" стала равной по шаблону:
				...
				| '1' | 'Без договора'              | 'ВА - Статья 1'   | ''       | ''            | ''                  | ''      | 'ВА - Здания' | 'Автоматически'           | '1'    | '3 000'          | 'Счет от поставщика * от *' | '60.02'        | '20%'        | '500,00' | '60.01'          | ''            | 'ВА - ЦФО 1 (АХО)' | ''            | ''            | ''            | 'Основной долг'                   | 'Заявка на оплату *; 3 000 RUB к *; ВА - Поставщик1; Оплата поставщику' | ''               | ''       |
