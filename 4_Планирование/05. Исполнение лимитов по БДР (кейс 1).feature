...
Сценарий: 05.01.1 Создаем Резервирование планов и лимитов по ЦФО 1 (АХО) на 1 квартал 2024 год
	...
	* Ввести новый документ
		...
		И элемент формы с именем 'ПериодОтчетаОкончание' стал равен '4 квартал 2024 г.'
...
Сценарий: 05.01.2 Создаем Заявку на расход				
	...
	* Заполнить аналитики учета и планирования 
		...
		И в таблице 'ТаблицаДвижений' в поле с именем 'ТаблицаДвиженийСумма' я ввожу текст '$СуммаЗнР$'	
		...
		Если в таблице 'ТаблицаДвижений' количество строк "больше" 1 тогда
			И в таблице 'ТаблицаДвижений' я перехожу к следующей строке
			И в таблице 'ТаблицаДвижений' я удаляю строку
			И в таблице 'ТаблицаДвижений' я удаляю строку
...
Сценарий: 05.01.3 Отражаем фактические данные об расходе	 		 		  		 		 		 	 
	...
	* Ввести Поступление на основание счета
		...
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			...
			И я перехожу к следующему реквизиту
			И в таблице 'Товары' я активизирую поле с именем 'ТоварыУХ_СтатьиИАналитики'
			...
			И Я в списке "Список" по полю "Наименование" ищу и выбираю элемент "ВА - Прочее" "По точному совпадению"
	...
	* Проверить движения ОФД	
		...
		Если '$$ЭтоУХ$$' Тогда
			Тогда таблица "ОперацииБюджетов" стала равной по шаблону:
				| 'N' | 'Период'                             | 'Вид бюджета'               | 'Объект планирования'                                                       | 'Состояние' | 'Аналитика структуры'             | 'Аналитика статей бюджетов'                                   | 'Аналитика партнеров'                                        | 'Приход расход' | 'Валюта' | 'Валюта лимитирования' | 'Сумма'     | 'Лимит'     | 'Сумма исполнено' | 'Лимит исполнено' | 'Источник версии' | 'Курс планирования' | 'Кратность планирования' | 'Курс лимитирования' | 'Кратность лимитирования' | 'Корректировка документа' |
				| '1' | '$$ЖелательнаяДатаРасхода$$ 0:00:00' | 'Бюджет доходов и расходов' | 'Заявка на расход *; 3 000 RUB *; ВА - Поставщик1; Расчеты с контрагентами' | ''          | 'ВА - ЦФО 1 (АХО); ВА - ООО Тест' | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | 'ВА - ООО Тест; ВА - Поставщик1; Без договора; Без договора' | 'Расход'        | 'RUB'    | 'RUB'                  | '-3 000,00' | '-3 000,00' | '3 000,00'        | '3 000,00'        | ''                | '1,0000'            | '1'                      | '1,0000'             | '1'                       | ''                        |
...
Сценарий: 05.02.3 Создаем Корректировку лимитов по заявке
	...
	* Обеспечить за счет переноса из другого периода
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'ВА - Здания' | '1 квартал 2024 г.' | 'Расход'      | 'ВА - Статья 1'  | 'ВА - ЦФО 1 (АХО)' |
		И в таблице "Список" я выбираю текущую строку
...
Сценарий: 05.03.3 Создаем Корректировку лимитов по заявке
	...
	* Обеспечить за счет переноса резерва из другого периода
		...
		Если флаг с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент1Использование' равен 'Да' Тогда
			И я снимаю флаг с именем 'КомпоновщикНастроекПользовательскиеНастройкиЭлемент1Использование'
		И в таблице "Список" я перехожу к строке:
				| 'Дата операции'     | 'Направление' | 'Проект'        | 'Статья бюджета' | 'ЦФО/Проект'       |
				| '2 квартал 2024 г.' | 'Расход'      | 'ВА - Проект 1' | 'ВА - Статья 4'  | 'ВА - ЦФО 1 (АХО)' |
