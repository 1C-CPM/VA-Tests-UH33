...
Сценарий: 10.06. [Тест1] Договор
	...
	* Создаем договор
		...
		Тогда таблица 'Номенклатура' стала равной:
			...
			| 'Товар1'       | 'кг'                | '100,000'    | '1,00000000'  | '20%'        | '1 576,11' |'131 342,54'     | '26 268,51' | '157 611,05'  |  'КАЗАХСТАН'            | 'КАЗАХСТАН'            | 'Статья БДДС 1' |'Статья БДР 1' | 'МестоПоставки1' | 'ЦФО1 Королевство Островов и Рек' |
...
Сценарий: 10.07. [Тест1] Заказ поставщику
	...
	* Товары
		...
		И в таблице 'СписокПотребностей' я перехожу к строке:
			...
			| 'Товар1'       | 'кг'                | '100,000'    | '1 576,11' | '157 611,00' |
		И в таблице 'СписокПотребностей' я нажимаю на кнопку с именем 'СписокПотребностейПеренестиПотребность'
		И я нажимаю на кнопку с именем 'ФормаОтправитьИЗакрыть'
...
Сценарий: 10.08. [Тест1] ПТУ
	...
	* Заказ поставщика
		...
		И в таблице 'Список' я перехожу к строке:
			...
			|'Поставщик1'|'Королевство Островов и Рек'|'RUB'             |'157 611,00'     |
		И в таблице 'Список' я выбираю текущую строку
		...
		Тогда таблица 'Товары' стала равной:
			...
			| 'Товар1'       | '100,000'    | '157 611,00' | 'кг'      | '1 576,11' |  '157 611,00' | '26 268,50' | 'Статья БДР 1;XXX1;YYY1;ZZZ1;SSS1;RRR1;WWW1'  | 'КАЗАХСТАН'            | 'КАЗАХСТАН'            |
...
Сценарий: 10.09. [Тест2] Закупочная процедура
	...
	* Проверяем подобранные товары 
		...
		Тогда таблица 'Номенклатура' стала равной:
			...
			| 'Товар2'       | '10 000,00' | 'упак'              | 'ЦФО1 Королевство Островов и Рек' | '100,000'    | 'МестоПоставки1' | 
...
Сценарий: 10.10. [Тест2] Предложение участников 1 EUR
		...
		* Товар 2
		...
		И в таблице 'УсловияПоНоменклатуреПоПериодам' я активизирую поле с именем 'УсловияПоНоменклатуреПоПериодамЦена'
		И в таблице 'УсловияПоНоменклатуреПоПериодам' я выбираю текущую строку
		И в таблице 'УсловияПоНоменклатуреПоПериодам' в поле с именем 'УсловияПоНоменклатуреПоПериодамЦена' я ввожу текст "8 100,00"
		И в таблице 'УсловияПоНоменклатуреПоПериодам' я завершаю редактирование строки
...
Сценарий: 10.13. [Тест2] Договор

Сценарий: 10.14. [Тест2] Заказ поставщику
