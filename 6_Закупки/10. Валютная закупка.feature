...
Сценарий: 10.06. [Тест1] Договор ДД9
	...
	* Создаем договор
		...
		Тогда таблица 'Номенклатура' стала равной:
			...
			| 'Товар1'       | 'кг'                | '100,000'    | '1,00000000'  | '22%'        | '7 266,03' | '605 502,50'    | '121 100,50' | '726 603,00'  |  'КАЗАХСТАН'            | 'КАЗАХСТАН'            | 'Статья БДДС 1' |'Статья БДР 1' | 'МестоПоставки1' | 'ЦФО1 Королевство Островов и Рек' |
...
Сценарий: 10.13. [Тест2] Договор ДД10
	...
	* Создаем договор
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Номенклатура' | 'Единица измерения' | 'Количество' | 'Коэффициент' | 'Ставка НДС' | 'Цена'      | 'Сумма без НДС' | 'Сумма НДС'  | 'Сумма с НДС'   |  'Страны происхождения' | 'Страна производителя' | 'Статья БДДС'   |'Статья БДР'   | 'Место поставки' | 'ЦФО'                             |
			| 'Товар2'       | 'Упак'              | '100,000'    | '1,00000000'  | '22%'        | '40 000,00' | '3 333 333,33'  | '666 666,67' | '4 000 000,00'  |  'КАЗАХСТАН'            | 'КАЗАХСТАН'            | 'Статья БДДС 1' |'Статья БДР 1' | 'МестоПоставки1' | 'ЦФО1 Королевство Островов и Рек' |
		И я перехожу к закладке с именем 'ГруппаГрафик'
		И в таблице 'ГрафикРасчетов' я нажимаю на кнопку с именем 'ГрафикРассчитатьПоГрафикуПоставок'
		И я нажимаю на кнопку с именем 'ФормаПровестиИЗакрыть'
		И я жду закрытия окна "С поставщиком (создание) *" в течение 20 секунд
