...
Сценарий: 10.01. Заявка на обеспечение
	...
	* Дата поставки	
		...
		И в поле с именем 'ДатаОперации' я ввожу текст "31.12.2026"
...
Сценарий: 10.02. [Тест1] Закупочная процедура
	...
	* Подбираем товары из заявки
		...
		И в таблице 'СписокПотребностей' я перехожу к строке:
			| "Единица измерения" | "Количество" | "Место поставки" | "Номенклатура" | "Организация"                | "Цена"      | "ЦФО потребитель"                 |
	...
	* Проверяем подобранные товары 
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Номенклатура' | 'Цена'      | 'Единица измерения' | 'Дата поставки' | 'ЦФО'                             | 'Количество' | 'Место поставки' | 
			| 'Товар1'       | '10 000,00' | 'кг'                | '31.12.2026'    | 'ЦФО1 Королевство Островов и Рек' | '100,000'    | 'МестоПоставки1' | 
...
Сценарий: 10.03. [Тест1] Предложение участников 1 EUR
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'Семь королевств'     |'Королевство Островов и Рек'| '2026 г.'        | 'Предмет договора (#тест10-1)' | 'Запрос котировок' |'Утвержден' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Закупочная процедура №*'
		...
		И в таблице "Лоты" я перехожу к строке:
			...
			| '2026 г.'        | 'Лотовая закупка' | 'Королевство Островов и Рек' | 'По конкурентным листам' | 'Предмет договора (#тест10-1)' | '1 000 000,00, RUB' |
...
Сценарий: 10.06. [Тест1] Договор ДД9
	...
	* Ищем протокол
		...
		И в таблице 'Список' я перехожу к строке:
			...
			| "Королевство Островов и Рек" | "Да"                 | "Семь королевств"     | "2026 г."        | "#10-1 Победитель определен, кворум имеется" | "Запрос котировок" |
	...
	* Создаем договор
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Номенклатура' | 'Единица измерения' | 'Количество' | 'Коэффициент' | 'Ставка НДС' | 'Цена'     | 'Сумма без НДС' | 'Сумма НДС'  | 'Сумма с НДС' |  'Страны происхождения' | 'Страна производителя' | 'Статья БДДС'   |'Статья БДР'   | 'Место поставки' | 'ЦФО'                             |
			| 'Товар1'       | 'кг'                | '100,000'    | '1,00000000'  | '22%'        | '7 266,03' | '595 576,23'    | '131 026,77' | '726 603,00'  |  'КАЗАХСТАН'            | 'КАЗАХСТАН'            | 'Статья БДДС 1' |'Статья БДР 1' | 'МестоПоставки1' | 'ЦФО1 Королевство Островов и Рек' |
...
Сценарий: 10.07. [Тест1] Заказ поставщику
	...
	* Добавление товаров по кнопке Подбор
...
//			И я нажимаю на кнопку с именем 'OK'
		...
		И я нажимаю на кнопку с именем 'ФормаПровестиИЗакрыть'
		Тогда открылось окно "Зак* поставщик*"
...
Сценарий: 10.08. [Тест1] ПТУ
	...
	* Заказ поставщика
		...
		Тогда таблица 'Товары' стала равной:
			...
			| 'Товар1'       | '100,000'    | '726 603,00' | 'кг'      | '7 266,03' |  '726 603,00' | '131 026,77' | 'КАЗАХСТАН'            | 'КАЗАХСТАН'            |
...
Сценарий: 10.09. [Тест2] Закупочная процедура
	...
	* Проверяем подобранные товары 
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Номенклатура' | 'Цена'      | 'Единица измерения' | 'ЦФО'                             | 'Количество' | 'Место поставки' | 
...
Сценарий: 10.10. [Тест2] Предложение участников 1 EUR
		...
		И в таблице "Список" я перехожу к строке:
			| 'Организатор закупки' |'Заказчик'                  | 'Период закупок' | 'Предмет договора'             | 'Способ закупки'     |'Статус'    |
			| 'Семь королевств'     |'Королевство Островов и Рек'| '2026 г.'        | 'Предмет договора (#тест10-2)' | 'Запрос предложений' |'Утвержден' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Закупочная процедура №*'
		...
		И в таблице "Лоты" я перехожу к строке:
			| 'Период закупок' | 'Вид закупки'     | 'Заказчик'                   | 'Метод оценки'           | 'Предмет договора'             | 'НМЦ'             |
			| '2026 г.'        | 'Лотовая закупка' | 'Королевство Островов и Рек' | 'По конкурентным листам' | 'Предмет договора (#тест10-2)' | '850 000,00, RUB' |
...
Сценарий: 10.13. [Тест2] Договор ДД10
	...
	* Ищем протокол
		...
		И в таблице 'Список' я перехожу к строке:
			| "Заказчик"                   | "Закупка состоялась" | "Организатор закупки" | "Период закупок" | "Результат работы комиссии"                  | "Способ закупки"     |
			| "Королевство Островов и Рек" | "Да"                 | "Семь королевств"     | "2026 г."        | "#10-2 Победитель определен, кворум имеется" | "Запрос предложений" |
	...
	* Создаем договор
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Номенклатура' | 'Единица измерения' | 'Количество' | 'Коэффициент' | 'Ставка НДС' | 'Цена'      | 'Сумма без НДС' | 'Сумма НДС'  | 'Сумма с НДС'   |  'Страны происхождения' | 'Страна производителя' | 'Статья БДДС'   |'Статья БДР'   | 'Место поставки' | 'ЦФО'                             |
			| 'Товар2'       | 'Упак'              | '100,000'    | '1,00000000'  | '22%'        | '40 000,00' | '3 278 688,52'  | '721 311,48' | '4 000 000,00'  |  'КАЗАХСТАН'            | 'КАЗАХСТАН'            | 'Статья БДДС 1' |'Статья БДР 1' | 'МестоПоставки1' | 'ЦФО1 Королевство Островов и Рек' |
		И я перехожу к закладке с именем 'ГруппаГрафик'
		И в таблице 'ГрафикРасчетов' я нажимаю на кнопку с именем 'ГрафикРассчитатьПоГрафикуПоставок'
		И я нажимаю на кнопку с именем 'ФормаПровестиИЗакрыть'
		И я жду закрытия окна "С поставщиком (создание) *" в течение 20 секунд
