...
Сценарий: 05.02. [ЗАПРОС ПРЕДЛОЖЕНИЙ] СПЗ 2
	...
	* Материал2
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал2", ЦФО = "Королевство Севера [223]", МестоПоставки = "МестоПоставки1", ДатаПоставки = "25.03.2026", Колво = "100", Цена = "1 200,00", СтавкаНДС = "22%", Проект = "Проект1"
	...
	* Материал3
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал3", ЦФО = "Королевство Севера [223]", МестоПоставки = "МестоПоставки3", ДатаПоставки = "18.05.2026", Колво = "150", Цена = "1 500,00", СтавкаНДС = "22%", Проект = "Проект1"
		И Я добавляю Статью БДДС в ТЧ "Номенклатура", Статья = "Статья БДДС 1", А1 = "AAA1", А2 = "BBB1", А3 = "CCC1", А4 = "DDD1", А5 = "EEE1", А6 = "FFF1"
		И Я добавляю Статью БДР в ТЧ "Номенклатура", Статья = "Статья БДР 1", А1 = "XXX1", А2 = "YYY1", А3 = "ZZZ1", А4 = "SSS1", А5 = "RRR1", А6 = "WWW1"

	* Убедимся, что таблица заполнена корректно

		Тогда таблица 'Номенклатура' стала равной:
			| 'Статья БДДС'   | 'Единица измерения' | 'Аналитика 4' | 'Номенклатура' | 'Дата поставки' | 'Количество' | 'Место поставки' | 'Коэффициент' | 'Период потребности' | 'Ставка НДС' | 'Сумма без НДС' | 'Цена первоначальная' | 'Сумма НДС' | 'Аналитика 2' | 'ЦФО'                      | 'Сумма с НДС' | 'Аналитика 5' | 'Проект'  | 'Аналитика 1' | 'Аналитика 3' | 'НМЦ'      | 'Аналитика 6' | 'Статья БДР'   |
			| 'Статья БДДС 1' | 'т'                 | 'SSS1'        | 'Материал2'    | '25.03.2026'    | '100,000'    | 'МестоПоставки1' | '1,00000000'  | 'Март 2026 г.'       | '22%'        | '98 360,66'     | '1 300,00'            | '21 639,34' | 'YYY1'        | 'Королевство Севера [223]' | '120 000,00'  | 'RRR1'        | 'Проект1' | 'XXX1'        | 'ZZZ1'        | '1 200,00' | 'WWW1'        | 'Статья БДР 1' |
			| 'Статья БДДС 1' | 'т'                 | 'SSS1'        | 'Материал3'    | '18.05.2026'    | '150,000'    | 'МестоПоставки3' | '1,00000000'  | 'Май 2026 г.'        | '22%'        | '184 426,23'    | '1 500,00'            | '40 573,77' | 'YYY1'        | 'Королевство Севера [223]' | '225 000,00'  | 'RRR1'        | 'Проект1' | 'XXX1'        | 'ZZZ1'        | '1 500,00' | 'WWW1'        | 'Статья БДР 1' |
...
Сценарий: 05.05. [ЗАПРОС ПРЕДЛОЖЕНИЙ] Закупочная процедура 2 / попозиционная
	...
	* Создаем процедуру
		...
		Тогда открылось окно 'Строки планов закупок (223-ФЗ)'
		И в таблице "Список" я перехожу к строке:
			| 'Валюта' | 'Заказчик'                 | 'Организатор'     |'Период закупок' | 'Предмет договора'                | 'Способ закупки'     | 'Статус'    | 'Сумма'      |
			| 'RUB'    | 'Королевство Севера [223]' | 'Семь королевств' |'2026 г.'        | 'Приобретение материалов, тест 2' | 'Запрос предложений' | 'Утвержден' | '345 000,00' |
...
Сценарий: 05.08. [ЗАПРОС ПРЕДЛОЖЕНИЙ] Предложение 1
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'Семь королевств'     |'Королевство Севера [223]'| '2026 г.'        | 'Приобретение материалов, тест 2' | 'Запрос предложений' |'Утвержден' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Закупочная процедура №*'
		...
		И в таблице "Лоты" я перехожу к строке:
			...
			| '2026 г.'        | 'Лотовая закупка' | 'Королевство Севера [223]' | 'По критериям (Настройка завершена)' | 'Приобретение материалов, тест 2' | '345 000,00, RUB' |
...
Сценарий: 05.11. [ЗАПРОС ПРЕДЛОЖЕНИЙ] Договор ДД2
	...
	* Открываем протокол
		...
		И в таблице 'Список' я перехожу к строке:
			...
			|'Королевство Севера [223]'|'Семь королевств'    |'2026 г.'       |'Запрос предложений'|'#2 Победитель определен, кворум имеется'|
...
Сценарий: 05.11. [ЗАПРОС ПРЕДЛОЖЕНИЙ] ПТУ (ДД2)
	...
	* Проверяем правильность заполнения строки
		...
		И в таблице "Товары" текущая строка равна:
			...
			| 'Материал2'    | '22%'   | '100,000'    | '100 000,00' | 'т'       | '1 000,00' | '100 000,00' | '16 666,67' | 'РОССИЯ'               |
		...
		И в таблице "Товары" текущая строка равна:
			| 'Номенклатура' | '% НДС' | 'Количество' | 'Всего'      | 'Единица' | 'Цена'     | 'Сумма'      | 'НДС'       | 'Страна происхождения' |
			| 'Материал3'    | '22%'   | '150,000'    | '180 000,00' | 'т'       | '1 200,00' | '180 000,00' | '30 000,00' | 'РОССИЯ'               |			
