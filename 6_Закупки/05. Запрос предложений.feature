...
Сценарий: 05.02. [ЗАПРОС ПРЕДЛОЖЕНИЙ] СПЗ 2
	...
	* Убедимся, что таблица заполнена корректно
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Статья БДДС'   | 'Единица измерения' | 'Номенклатура' | 'Дата поставки' | 'Количество' | 'Место поставки' | 'Коэффициент' | 'Период потребности' | 'Ставка НДС' | 'Сумма без НДС' | 'Цена первоначальная' | 'Сумма НДС' | 'ЦФО'                      | 'Сумма с НДС' | 'Проект'  | 'НМЦ'      | 'Статья БДР'   |
			| 'Статья БДДС 1' | 'т'                 | 'Материал2'    | '25.03.2026'    | '100,000'    | 'МестоПоставки1' | '1,00000000'  | 'Март 2026 г.'       | '22%'        | '98 360,66'     | '1 300,00'            | '21 639,34' | 'Королевство Севера [223]' | '120 000,00'  | 'Проект1' | '1 200,00' | 'Статья БДР 1' |
			| 'Статья БДДС 1' | 'т'                 | 'Материал3'    | '18.05.2026'    | '150,000'    | 'МестоПоставки3' | '1,00000000'  | 'Май 2026 г.'        | '22%'        | '184 426,23'    | '1 500,00'            | '40 573,77' | 'Королевство Севера [223]' | '225 000,00'  | 'Проект1' | '1 500,00' | 'Статья БДР 1' |
...
Сценарий: 05.11. [ЗАПРОС ПРЕДЛОЖЕНИЙ] ПТУ (ДД2)
	...
	* Проверяем правильность заполнения строки
		...
		И в таблице "Товары" текущая строка равна:
			| 'Номенклатура' | '% НДС' | 'Количество упаковок' | 'Всего'      | 'Упаковка единица измерения' | 'Цена'     | 'Сумма'      | 'НДС'       | 'Страна происхождения' |
