...
Сценарий: 09.02. [ЗАЯВКА -> ЛОТ;  Королевство простора] Закупочная процедура 
	...
	* Метод оценки
		...
		И из выпадающего списка с именем 'МетодОценки' я выбираю точное значение "По конкурентным листам"	
	...
	* Проверяем подобранные товары 
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Номенклатура' | 'Цена'      | 'Единица измерения' |  'Дата поставки' | 'Потребитель (ЦФО)'         | 'Количество' | 'Место поставки' | 'Сумма без НДС' | 'Ставка НДС' | 'Сумма НДС'  | 'Сумма с НДС'  | 'Статья БДДС'   |'Статья БДР'    |
