...
Сценарий: 11.02. [ЗАПРОС КОТИРОВОК] Закупочная процедура 1 / лотовая
	...
	* Создаем процедуру
		...
		И в таблице "Список" я перехожу к строке:
			...
			|'2026 г.'       | 'Королевство Севера [223]' | 'Семь королевств' | 'Запрос котировок' | 'Приобретение материалов, тест 4' | '414 000,00' |  'RUB'  | 'Утвержден' |
...
Сценарий: 11.03. [ЗАПРОС КОТИРОВОК] Предложение 1
		...
		И в таблице "Лоты" я перехожу к строке:
			...
			| '2026 г.'        | 'Лотовая закупка' | 'Королевство Севера [223]' | 'По конкурентным листам' | 'Приобретение материалов, тест 4' | '414 000,00, RUB' |
...
Сценарий: 11.06. [ЗАПРОС КОТИРОВОК] Договор ДД6
	...
	* Контроль графика поставок
		...
		Тогда таблица 'Номенклатура' стала равной:
			...
			| 'Материал2'    | 'Королевство Севера [223]' | 'МестоПоставки1' | '100,000'    | 'т'                 | '800,00'   | 'РОССИЯ'               | '1,00000000'  | '15.08.2026'    | '22%'        | '80 000,00'     | '96 000,00'   | 'Проект1' | '16 000,00' | 'РОССИЯ'               |
			| 'Материал3'    | 'Королевство Севера [223]' | 'МестоПоставки3' | '150,000'    | 'т'                 | '1 400,00' | 'РОССИЯ'               | '1,00000000'  | '20.05.2026'    | '22%'        | '210 000,00'    | '252 000,00'  | 'Проект1' | '42 000,00' | 'РОССИЯ'               |
...
Сценарий: 11.07. [ЗАПРОС КОТИРОВОК] ПТУ по договору ДД6
	...
	* Проверяем правильность заполнения строки
		...
		И в таблице "Товары" текущая строка равна:
			...
			| 'Материал2'    | '22%'   | '100,000'    | 'т'       | '800,00'   | '16 000,00' | 'РОССИЯ'                | '80 000,00'  | '96 000,00'  |
		...
		И в таблице "Товары" текущая строка равна:
			| 'Номенклатура' | '% НДС' | 'Количество' | 'Единица' | 'Цена'     | 'НДС'       | 'Страна происхождения'  | 'Сумма'      | 'Всего'      |
			| 'Материал3'    | '22%'   | '150,000'    | 'т'       | '1 400,00' | '42 000,00' | 'РОССИЯ'                | '210 000,00' | '252 000,00' |
