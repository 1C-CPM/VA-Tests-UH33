...
Сценарий: 11.00. Полные права: Изменение закупочных цен
...
Сценарий: 11.01. [ЗАПРОС КОТИРОВОК] СПЗ 1
	...
	* Закладка Потребности
		...
		* Номенклатура1
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал2", ЦФО = "Королевство Севера [223]", МестоПоставки = "МестоПоставки1", ДатаПоставки = "15.08.2026", Колво = "100", Цена = "1 200,00", СтавкаНДС = "22%", Проект = "Проект1"
	...
	* Номенклатура2
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал3", ЦФО = "Королевство Севера [223]", МестоПоставки = "МестоПоставки3", ДатаПоставки = "20.05.2026", Колво = "150", Цена = "1 500,00", СтавкаНДС = "22%", Проект = "Проект1"
		И Я добавляю Статью БДДС в ТЧ "Номенклатура", Статья = "Статья БДДС 1", А1 = "AAA1", А2 = "BBB1", А3 = "CCC1", А4 = "DDD1", А5 = "EEE1", А6 = "FFF1"
		И Я добавляю Статью БДР в ТЧ "Номенклатура", Статья = "Статья БДР 1", А1 = "XXX1", А2 = "YYY1", А3 = "ZZZ1", А4 = "SSS1", А5 = "RRR1", А6 = "WWW1"
...
Сценарий: 11.02. [ЗАПРОС КОТИРОВОК] Закупочная процедура 1 / лотовая
	...
	* Создаем процедуру
		...
		Тогда открылось окно 'Строки планов закупок (223-ФЗ)'
		И в таблице "Список" я перехожу к строке:
			|'Период закупок'| 'Заказчик'                 | 'Организатор'     | 'Способ закупки'   | 'Предмет договора'                | 'Сумма'      |'Валюта' | 'Статус'    |
			|'2026 г.'       | 'Королевство Севера [223]' | 'Семь королевств' | 'Запрос котировок' | 'Приобретение материалов, тест 4' | '420 900,00' |  'RUB'  | 'Утвержден' |
...
Сценарий: 11.03. [ЗАПРОС КОТИРОВОК] Предложение 1
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'Семь королевств'     |'Королевство Севера [223]'| '2026 г.'        | 'Приобретение материалов, тест 4' | 'Запрос котировок' |'Утвержден' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Закупочная процедура №*'
		...
		И в таблице "Лоты" я перехожу к строке:
			...
			| '2026 г.'        | 'Лотовая закупка' | 'Королевство Севера [223]' | 'По конкурентным листам' | 'Приобретение материалов, тест 4' | '420 900,00, RUB' |
...
Сценарий: 11.04. [ЗАПРОС КОТИРОВОК] Предложение 2
...
Сценарий: 11.05. [ЗАПРОС КОТИРОВОК] Протокол
	...
	* Результат
		...
		И в поле с именем 'РезультатРаботыКомиссии' я ввожу текст "#4 Победитель определен, кворум имеется"
		И я перехожу к следующему реквизиту
...
Сценарий: 11.06. [ЗАПРОС КОТИРОВОК] Договор ДД6
	...
	* Открываем протокол
		...
		И в таблице 'Список' я перехожу к строке:
			...
			|'Королевство Севера [223]'|'Семь королевств'    |'2026 г.'       |'Запрос котировок' |'#4 Победитель определен, кворум имеется'|
	...
	* Контроль НДС (НДС сверху)

		Тогда элемент формы с именем 'СуммаВключаетНДС' стал равен "0"
	...
	* Контроль графика поставок
		...
		Тогда таблица 'Номенклатура' стала равной:
			| 'Номенклатура' | 'ЦФО'                      | 'Место поставки' | 'Количество' | 'Единица измерения' | 'Цена'     | 'Страна производителя' | 'Коэффициент' | 'Дата поставки' | 'Ставка НДС' | 'Сумма без НДС' | 'Сумма с НДС' | 'Проект'  | 'Сумма НДС' | 'Страны происхождения' |
			| 'Материал2'    | 'Королевство Севера [223]' | 'МестоПоставки1' | '100,000'    | 'т'                 | '1 100,00' | 'РОССИЯ'               | '1,00000000'  | '15.08.2026'    | '22%'        | '110 000,00'    | '134 200,00'  | 'Проект1' | '24 200,00' | 'РОССИЯ'               |
			| 'Материал3'    | 'Королевство Севера [223]' | 'МестоПоставки3' | '150,000'    | 'т'                 | '1 300,00' | 'РОССИЯ'               | '1,00000000'  | '20.05.2026'    | '22%'        | '195 000,00'    | '237 900,00'  | 'Проект1' | '42 900,00' | 'РОССИЯ'               |
	...
	* Записываем и проводим договор
		...
		И я нажимаю на кнопку с именем 'ФормаЗаписать'
		Тогда открылось окно 'ДД6 от * (С поставщиком) вер. * от *'
		И я нажимаю на кнопку с именем 'ФормаПровестиИЗакрыть'
		И я жду закрытия окна 'ДД6 от * (С поставщиком) вер. * от *' в течение 20 секунд
	
	* Закрываем все окна клиентского приложения

		И я закрываю все окна клиентского приложения

Сценарий: 11.07. [ЗАПРОС КОТИРОВОК] ПТУ по договору ДД6
	...
	* Проверяем правильность заполнения строки
		...
		И в таблице "Товары" текущая строка равна:
			...
			| 'Материал2'    | '22%'   | '100,000'    | 'т'       | '1 100,00' | '24 200,00' | 'РОССИЯ'                | '110 000,00' | '134 200,00' |
		...
		И в таблице "Товары" текущая строка равна:
			| 'Номенклатура' | '% НДС' | 'Количество' | 'Единица' | 'Цена'     | 'НДС'       | 'Страна происхождения'  | 'Сумма'      | 'Всего'      |
			| 'Материал3'    | '22%'   | '150,000'    | 'т'       | '1 300,00' | '42 900,00' | 'РОССИЯ'                | '195 000,00' | '237 900,00' |
