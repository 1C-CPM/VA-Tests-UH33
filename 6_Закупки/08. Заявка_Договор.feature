...
Сценарий: 08.02. [ЗАЯВКА -> ДОГОВОР] Договор ДД5 Королевство Гор и Долин
	...
	* Проверяем подобранные данные
		...
		Тогда таблица 'Номенклатура' стала равной:
				| 'Цена'      | 'Единица измерения' |  'Номенклатура' | 'Количество' | 'Место поставки' | 'Дата поставки' | 'Потребитель (ЦФО)'     | 'Ставка НДС' | 'Сумма без НДС' | 'Сумма НДС' | 'Сумма с НДС' | 'Проект'  | 'Статья БДДС'   | 'Статья БДР'   |
	...
	* Добавляем товар вручную 
		...
		И в таблице 'Номенклатура' из выпадающего списка с именем 'НоменклатураНоменклатура' я выбираю точное значение "Номенклатура"
		Тогда открылось окно "Номенклатура"
		И в таблице 'Список' я перехожу к строке:
