...
Сценарий: 04.01. [ЗАПРОС КОТИРОВОК] СПЗ 1
	...
	* 2026
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал1", ЦФО = "Королевство Севера [223]", МестоПоставки = "МестоПоставки1", ДатаПоставки = "15.08.2026", Колво = "100", Цена = "1 200,00", СтавкаНДС = "22%", Проект = "Проект1"
	...
	* 2027
		...
		И Я добавляю Номенклатуру в ТЧ ="Номенклатура", Номенклатура = "Материал2", ЦФО = "Королевство Севера [223]", МестоПоставки = "МестоПоставки1", ДатаПоставки = "20.07.2027", Колво = "250", Цена = "1 400,00", СтавкаНДС = "22%", Проект = "Проект1"
		И Я добавляю Статью БДДС в ТЧ "Номенклатура", Статья = "Статья БДДС 1", А1 = "AAA1", А2 = "BBB1", А3 = "CCC1", А4 = "DDD1", А5 = "EEE1", А6 = "FFF1"
		И Я добавляю Статью БДР в ТЧ "Номенклатура", Статья = "Статья БДР 1", А1 = "XXX1", А2 = "YYY1", А3 = "ZZZ1", А4 = "SSS1", А5 = "RRR1", А6 = "WWW1"
...
Сценарий: 04.02. [ЗАПРОС КОТИРОВОК] Закупочная процедура 1 / лотовая
	...
	* Создаем процедуру
		...
		Тогда открылось окно 'Строки планов закупок (223-ФЗ)'
		И в таблице "Список" я перехожу к строке:
			|'Период закупок'| 'Заказчик'                 | 'Организатор'     | 'Способ закупки'   | 'Предмет договора'                | 'Статус'    | 'Сумма'      |'Валюта' | 'Статус'    |
			|'2026 г.'       | 'Королевство Севера [223]' | 'Семь королевств' | 'Запрос котировок' | 'Приобретение материалов, тест 1' | 'Утвержден' | '470 000,00' |  'RUB'  | 'Утвержден' |
...
Сценарий: 04.03. [ЗАПРОС КОТИРОВОК] Предложение 1
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'Семь королевств'     |'Королевство Севера [223]'| '2026 г.'        | 'Приобретение материалов, тест 1' | 'Запрос котировок' |'Утвержден' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Закупочная процедура №*'
		...
		И в таблице "Лоты" я перехожу к строке:
			...
			| '2026 г.'        | 'Лотовая закупка' | 'Королевство Севера [223]' | 'По конкурентным листам' | 'Приобретение материалов, тест 1' | '470 000,00, RUB' |
...
Сценарий: 04.05. [ЗАПРОС КОТИРОВОК] Протокол
	...
	* Выбираем победителя
		...
		И я нажимаю на кнопку с именем 'ФормаРежимОценки'
...
Сценарий: 04.06. [ЗАПРОС КОТИРОВОК] Договор ДД1
	...
	* Открываем протокол
		...
		И в таблице 'Список' я перехожу к строке:
			...
			|'Королевство Севера [223]'|'Семь королевств'    |'2026 г.'       |'Запрос котировок'|'#1 Победитель определен, кворум имеется'|
...
Сценарий: 04.07. [ЗАПРОС КОТИРОВОК] ПТУ по договору ДД1
	...
	* Проверяем правильность заполнения строки
		...
		И в таблице "Товары" текущая строка равна:
			| 'Номенклатура' | '% НДС' | 'Количество упаковок' | 'Упаковка единица измерения' | 'Цена'     | 'НДС'       | 'Страна происхождения'  | 'Сумма'      | 'Всего'      |
			| 'Материал1'    | '22%'   | '100,000'             | 'т'                          | '1 000,00' | '18 032,79' | 'РОССИЯ'                | '100 000,00' | '100 000,00' |
		...
		И в таблице "Товары" текущая строка равна:
			| 'Номенклатура' | '% НДС' | 'Количество упаковок' | 'Упаковка единица измерения' | 'Цена'     | 'НДС'       | 'Страна происхождения'  | 'Сумма'      | 'Всего'      |
			| 'Материал2'    | '22%'   | '250,000'             | 'т'                          | '1 200,00' | '54 098,36' | 'РОССИЯ'                | '300 000,00' | '300 000,00' |
