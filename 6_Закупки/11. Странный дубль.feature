...
//@UH33_Purchases
//
//Функционал: 14. Запрос котировок [от спз до пту]: НДС сверху
//
...
//
//Сценарий: 14.00. Ограниченные права: инициализация
//
//	И Я запоминаю значение выражения 'СокрЛП(ТекущаяДата())' в переменную "$$ТекущаяДата$$"	
//	
//	
//Сценарий: 14.01. [ЗАПРОС КОТИРОВОК] СПЗ 4
//
...
//
//Сценарий: 14.02. [ЗАПРОС КОТИРОВОК] Закупочная процедура 1 / лотовая
//
//	* Создаем процедуру
//	
//		И В командном интерфейсе я выбираю 'Корпоративные закупки' 'Строки планов закупок'
...
//
//Сценарий: 14.03. [ЗАПРОС КОТИРОВОК] Предложение 1
//
...
//
//Сценарий: 14.04. [ЗАПРОС КОТИРОВОК] Предложение 2
//
//		И В командном интерфейсе я выбираю 'Корпоративные закупки' 'Закупочные процедуры'
//		Тогда открылось окно 'Закупочные процедуры'
//
...
//		И я закрываю все окна клиентского приложения		 
//
//Сценарий: 14.05. [ЗАПРОС КОТИРОВОК] Протокол 000000003
//
...
//		И я закрываю все окна клиентского приложения
//			
//Сценарий: 14.06. [ЗАПРОС КОТИРОВОК] Договор ДД5
//
//	* Открываем протокол
//
...
//			|'Королевство Севера [223]'|'Семь королевств'    |'2025 г.'       |'000000001'|
//
...
//		И я закрываю все окна клиентского приложения
//
//Сценарий: 14.07. [ЗАПРОС КОТИРОВОК] ПТУ по договору ДД5
//
//	* Запоминаем текущую дату
//		
//		И Я запоминаю значение выражения 'СокрЛП(ТекущаяДата())' в переменную "ТекущаяДата"
