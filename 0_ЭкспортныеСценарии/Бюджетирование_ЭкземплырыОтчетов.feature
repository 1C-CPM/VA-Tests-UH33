...
Сценарий: Расшифровка по правилу расчета показателя в ячейке 'TheCell' равна макету "ThePath"
	...
	Дано Табличный документ 'ПолеТабличногоДокумента' равен макету "[ThePath]"

Сценарий: Для ячейки 'TheCell' адрес базового значения из макета "ThePath" равен 'TheBaseCell'

	И в табличном документе 'ПолеТабличногоДокумента' я перехожу к ячейке '[TheCell]'
	И в табличном документе 'ПолеТабличногоДокумента' я делаю двойной клик на текущей ячейке
	Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "[ThePath]"
	И я запоминаю адрес текущей ячейки 'ПолеТабличногоДокументаМакет' в переменную 'ТекущийАдрес'
	Тогда переменная 'ТекущийАдрес' имеет значение '[TheBaseCell]'
	И я закрываю текущее окно

Сценарий: Я сохраняю настройку списка экземпляров отчетов 'TheName' вид отчета 'TheReportKind' организация 'TheBusinessUnit' период 'ThePeriod' сценарий 'TheScenario' группа пользователей 'TheUserGroup'
	...
	* Установим фильтры
		...
		И в таблице 'Отбор' из выпадающего списка с именем 'ОтборПравоеЗначение' я выбираю по строке '[TheBusinessUnit]'
		...
		И в таблице 'Отбор' из выпадающего списка с именем 'ОтборПравоеЗначение' я выбираю по строке '[TheScenario]'
		
		И в таблице 'Отбор' я перехожу к строке:
			| "Вид сравнения" | "Использование" | "ПОЛЕ"       | "Режим отображения" |
			| "Равно"         | "Нет"           | "Вид отчета" | "Быстрый доступ"    |
		И в таблице 'Отбор' из выпадающего списка с именем 'ОтборПравоеЗначение' я выбираю по строке '[TheReportKind]'

	* Сохраним настройку
		Когда открылось окно "Экземпляры отчетов"
		И я нажимаю на кнопку с именем 'СохранитьНастройкуКак'	
		И Я в списке "Сохраненные настройки" по полю "Наименование" ищу элемент '[TheName]' "По точному совпадению"
		И Пока в таблице 'Список' количество строк 'больше' 0 Тогда
			...
			Тогда открылось окно "[TheName] (Вариант настройки)"
			И в поле с именем 'Наименование' я ввожу значение переменной 'УИД'
			И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
			И я жду закрытия окна "[TheName] *" в течение 20 секунд
	...
	* Настраиваем пользователей
		Когда открылось окно "[TheName] (Вариант настройки)"
		...
		И Я в списке "Выбор группы пользователей" по полю "Наименование" ищу и выбираю элемент '[TheUserGroup]' "По точному совпадению"
		Тогда открылось окно "[TheName] (Вариант настройки) *"
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		И я жду закрытия окна "[TheName] (Вариант настройки) *" в течение 20 секунд