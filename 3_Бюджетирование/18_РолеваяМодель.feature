...
Структура сценария: 18.04 Создание экземпляров отчетов
	...
	И Я ввожу значение '80' в ячейку 'R6C2'	
	Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_80.mxl'
...
Структура сценария: 18.05 Создание шаблонов корректировки значений показателей
...
Структура сценария: 18.06 Создание корректировок значений показателей
...
Сценарий: 18.07 Создание двойного шаблона корректировки значений показателей	
...
Сценарий: 18.08 Создание двойной корректировки значений показателей	
...
Сценарий: 18.11 Проверка корректировок значений покзаталей для "Бюджетирование1"
	...
	* Открываем список с отборами
		И В командном интерфейсе я выбираю "Бюджетирование, отчетность и анализ" "Корректировки значений показателей"
		Когда открылось окно "Корректировки значений показателей"
	...
	* Открываем документ без доступа
		И в таблице 'Список' я перехожу к строке:
			...
			| "Меркурий ООО" | "ВА - Основной сценарий" | "ВА - Проверка настроек доступа (несколько)" | "Ручной ввод"                |
		И в таблице 'Список' я выбираю текущую строку
		Тогда открылось окно "Корректировки значений показателей"
...
Сценарий: 18.12 Проверка экземпляров отчетов для "Бюджетирование1"
	...
	* Отменяем проведение корректировки
		...
		Когда открылось окно "Корректировки значений показателей"
		И я закрываю окно "Корректировки значений показателей"

	* Проверяем отмену корректировки
		И В командном интерфейсе я выбираю "Бюджетирование, отчетность и анализ" "Экземпляры отчетов"
		И в таблице 'Список' я перехожу к строке:
			| "Вид отчета"                              | "Организация"  | "Период отчета"  | "Период окончания" | "Сценарий"               | "Статус"    | "Проект"               | "Аналитика 1"   | "Основная валюта" |
			| "ВА - Проверка настроек доступа (запись)" | "Меркурий ООО" | "Январь 2024 г." | "Март 2024 г."     | "ВА - Основной сценарий" | "Черновик"  | "ВА - Основной проект" | "Администрация" | "RUB"             |
	...
	* Проверяем ввод новой версии значений
	...
	* Проверяем удаление версии значений
	...
	* Снова проводим корректировку
		И В командном интерфейсе я выбираю "Бюджетирование, отчетность и анализ" "Корректировки значений показателей"
		Когда открылось окно "Корректировки значений показателей"	
		И в таблице 'Список' я перехожу к строке:
			| "Организация" | "Сценарий"               | "Шаблон корректировки" | "Способ ввода корректировки" |
			| "Меркурий ООО"| "ВА - Основной сценарий" | ""                     | "Ручной ввод"                |
		И я нажимаю на кнопку с именем 'СписокКонтекстноеМенюПровести'
		Когда открылось окно "Корректировки значений показателей"
		И я закрываю окно "Корректировки значений показателей"
		
	* Проверяем значения экземпляра
		И В командном интерфейсе я выбираю "Бюджетирование, отчетность и анализ" "Экземпляры отчетов"
		И в таблице 'Список' я перехожу к строке:
			| "Вид отчета"                              | "Организация"  | "Период отчета"  | "Период окончания" | "Сценарий"               | "Статус"    | "Проект"               | "Аналитика 1"   | "Основная валюта" |
			| "ВА - Проверка настроек доступа (запись)" | "Меркурий ООО" | "Январь 2024 г." | "Март 2024 г."     | "ВА - Основной сценарий" | "Черновик"  | "ВА - Основной проект" | "Администрация" | "RUB"             |
...
Сценарий: 18.16 Проверка СТ для пользователя "Бюджетирование2"
	...
	* Проверяем удаление версии корректировки
		И я нажимаю на кнопку с именем 'РаботаСВерсиями'
		Тогда открылось окно "Управление версиями показателей"
		И я нажимаю на кнопку с именем 'СписокУдалитьВерсии'
		Тогда в логе сообщений TestClient есть строки:
			|"Выделенные версии недоступны для удаления"|
		И Я закрываю окно "Управление версиями показателей"
		Тогда открылось окно '$ЗаголовокОкна$'
	...
	* Проверяем изменение значения
		...
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_СТ_80.mxl'
...
Сценарий: 18.20 Тест интерфейса для пользователя "Бюджетирование3" - Панель разделов и функций
	...
	* Панель функций
		...
		И панель функций равна
			...
			| "Корректировки значений показателей" |
...
Сценарий: 18.25 Тест интерфейса для пользователя "Бюджетирование1" - Корректировка значений показателей
	...
	* Проверяем кнопки согласования	
		...
		И я нажимаю на кнопку с именем 'ФормаОбщаяКомандаУниверсальноеСогласование_СогласоватьДокумент'
		Тогда в логе сообщений TestClient есть строки по шаблону:
			|"Документ Корректировка значений показателей * не проведён. Отправка на согласование отменена."|
