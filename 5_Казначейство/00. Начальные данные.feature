...
Сценарий: 01.3. Начисление заработной платы
...
Сценарий: 01.4. Курсы валют для целей планирования	
...
Сценарий: 01.5. Виды подтверждающих документов по видам операций

	* Включаем функциональную опцию
		И В командном интерфейсе я выбираю "Казначейство" "Настройки казначейства"
		Тогда открылось окно "Казначейство"
		И я разворачиваю группу с именем 'ГруппаЗаявкиИПлатежи'
		Если флаг с именем 'ИспользоватьПодтверждающиеДокументы' равен 'Нет' тогда
			И я устанавливаю флаг с именем 'ИспользоватьПодтверждающиеДокументы'
			И пауза 2
		И Я закрываю окно "Казначейство"
								
	* Открываем форму настройки
		И В командном интерфейсе я выбираю "Казначейство" "Виды подтверждающих документов по видам операций"
		Тогда открылось окно "Виды подтверждающих документов по видам операций"

	* Добавим условие на вид подтверждающего документа
		И в таблице 'СписокВидовОпераций' я перехожу к строке:
			| "Вид операции"          |
			| "БДДС, Расход" |
		И в таблице 'СписокВидовОпераций' я перехожу на один уровень вниз
		И в таблице 'СписокВидовОпераций' я перехожу к строке по шаблону:
			| "Вид операции"          |
			| "Оплата поставщику*" |
		И в таблице 'Список' я нажимаю на кнопку с именем 'СписокСоздатьВиды'
		Тогда открылось окно "Добавление вида подтверждающего документа"
		И я нажимаю на кнопку создать поля с именем 'ВидПодтверждающегоДокумента'
		Тогда открылось окно "Вид документа (создание)"
		И я изменяю флаг с именем 'ОбъектСистемы'
		И я нажимаю кнопку выбора у поля с именем 'ОбъектБД'
		И Я в таблице '' по полю '' ищу значение 'Документ ИБ' 	
		...
		И в таблице 'КомпоновщикНастроекНастройкиОтбор' из выпадающего списка с именем 'КомпоновщикНастроекНастройкиОтборЛевоеЗначение' я выбираю по строке "Способ оплаты"
		...
		И в таблице 'КомпоновщикНастроекНастройкиОтбор' из выпадающего списка с именем 'КомпоновщикНастроекНастройкиОтборПравоеЗначение' я выбираю точное значение "Третьему лицу"
		И в таблице 'КомпоновщикНастроекНастройкиОтбор' я завершаю редактирование строки
		И из выпадающего списка с именем 'Наименование' я выбираю точное значение 'Способ оплаты Равно "Третьему лицу"'
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		И я жду закрытия окна "Вид документа (создание) *" в течение 20 секунд
		Тогда открылось окно "Добавление вида подтверждающего документа"
		И я устанавливаю флаг с именем 'Обязателен'
		И я нажимаю на кнопку с именем 'ФормаСоздать'
		Тогда таблица 'Список' стала равной:
			| 'Вид подтверждающего документа'            | 'Обязателен' |
			| '(98) Способ оплаты Равно "Третьему лицу"' | 'Да'         |
